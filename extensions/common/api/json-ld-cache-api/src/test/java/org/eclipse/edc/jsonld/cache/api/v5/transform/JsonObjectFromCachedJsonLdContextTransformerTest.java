...
class JsonObjectFromCachedJsonLdContextTransformerTest {

    private final JsonObjectFromCachedJsonLdContextTransformer transformer =
            new JsonObjectFromCachedJsonLdContextTransformer(Json.createBuilderFactory(emptyMap()));
    private final TransformerContext context = new TransformerContextImpl(mock());

    @Test
    void shouldTransformToJson() {
        var cached = CachedJsonLdContext.Builder.newInstance()
                .id("id1")
        ...
        assertThat(result.getString(TYPE)).isEqualTo(CACHED_JSON_LD_CONTEXT_TYPE_IRI);
        assertThat(result.getString(CACHED_JSON_LD_CONTEXT_URL_IRI)).isEqualTo("https://example.com/context.jsonld");
        assertThat(result.getString(CACHED_JSON_LD_CONTEXT_PULL_STRATEGY_IRI)).isEqualTo("NEVER");

        var content = result.getJsonObject(CACHED_JSON_LD_CONTEXT_CONTENT_IRI);
    ...
    void shouldTransformToJson_whenContentAbsent() {
        ...
        assertThat(result).isNotNull();
        assertThat(result.getString(CACHED_JSON_LD_CONTEXT_PULL_STRATEGY_IRI)).isEqualTo("IF_NOT_PRESENT");
        assertThat(result.get(CACHED_JSON_LD_CONTEXT_CONTENT_IRI)).isNull();
    }
