...
import static org.eclipse.edc.iam.identitytrust.transform.TestData.EXAMPLE_JWT_VP;
import static org.eclipse.edc.iam.identitytrust.transform.TestData.EXAMPLE_JWT_VP_EMPTY_CREDENTIALS_ARRAY;
import static org.eclipse.edc.iam.identitytrust.transform.TestData.EXAMPLE_JWT_VP_SINGLE_VC;
import static org.eclipse.edc.iam.identitytrust.transform.TestData.EXAMPLE_JWT_VP_WITH_LDP_VC;
import static org.eclipse.edc.iam.identitytrust.transform.TestData.EXMPLE_JWT_VP_NO_VP_CLAIM;
...
class JwtToVerifiablePresentationTransformerTest {
    ...
    private final JwtToVerifiablePresentationTransformer transformer = new JwtToVerifiablePresentationTransformer(monitor, MAPPER);
    ...
    private final JsonObjectToVerifiableCredentialTransformer credentialTransformerJson = new JsonObjectToVerifiableCredentialTransformer();
    ...
    void transform_containsLdpVc() {
        when(context.transform(isA(JsonObject.class), eq(VerifiableCredential.class)))
                .thenAnswer(a -> credentialTransformerJson.transform(a.getArgument(0), context));
