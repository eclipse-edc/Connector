...
import org.eclipse.dataspaceconnector.dataplane.cloud.http.HttpTestFixtures;
...
class CloudHttpDataSourceFactoryTest {
    ...
    private CloudHttpDataSourceFactory factory;

    @BeforeEach
    void setUp() {
        vaultMock = mock(Vault.class);
        factory = new CloudHttpDataSourceFactory(HTTP_CLIENT, RETRY_POLICY, MONITOR, vaultMock);
    }
    ...
    private static Stream<Arguments> provideTestInstances() {
        ...
        var put = TestInstance.newInstance()
                .method("PUT", true)
    ...
    private static final class TestInstance {
        ...
        private TestInstance() {
            ...
            props = new HashMap<>();
            address = DataAddress.Builder.newInstance().type(CloudHttpDataAddressSchema.TYPE);
