...
class ManagementApiSchemaTest {
    ...
    void contractAgreementExample() throws JsonProcessingException {
        ...
        assertThat(expanded.getJsonArray(TYPE).getString(0)).isEqualTo(CONTRACT_AGREEMENT_TYPE);
        assertThat(expanded.getJsonArray(CONTRACT_AGREEMENT_ASSET_ID).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_AGREEMENT_PROVIDER_ID).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_AGREEMENT_CONSUMER_ID).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_AGREEMENT_SIGNING_DATE).getJsonObject(0).getJsonNumber(VALUE).longValue()).isGreaterThan(0);
        assertThat(expanded.getJsonArray(CONTRACT_AGREEMENT_POLICY).getJsonObject(0)).isNotNull();
    }
    ...
    void contractNegotiationExample() throws JsonProcessingException {
        ...
        assertThat(expanded.getString(ID)).isNotBlank();
        assertThat(expanded.getJsonArray(TYPE).getString(0)).isEqualTo(CONTRACT_NEGOTIATION_TYPE);
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_CREATED_AT).getJsonObject(0).getJsonNumber(VALUE).longValue()).isGreaterThan(0);
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_NEG_TYPE).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_PROTOCOL).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_COUNTERPARTY_ID).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_COUNTERPARTY_ADDR).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_STATE).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_AGREEMENT_ID).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_ERRORDETAIL).getJsonObject(0).getString(VALUE)).isNotBlank();
        assertThat(expanded.getJsonArray(CONTRACT_NEGOTIATION_CALLBACK_ADDR)).asList().isNotEmpty();
    }
    ...
    void policyExample() throws JsonProcessingException {
        ...
        assertThat(expanded.getString(ID)).isNotBlank();
        assertThat(expanded.getJsonArray(TYPE).getString(0)).isNotBlank();
        assertThat(expanded.getJsonArray(ODRL_PERMISSION_ATTRIBUTE).size()).isGreaterThan(0);
    }
