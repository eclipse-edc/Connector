...
class ContractDefinitionServiceImplTest {
    ...
    void findById_filtersById() {
        ...
        when(store.findById("participantContextId", definition.getId())).thenReturn(definition);

        var result = service.findById("participantContextId", definition.getId());
    ...
    void findById_returnsNullIfNotFound() {
        when(store.findById(anyString(), anyString())).thenReturn(null);

        var result = service.findById("participantContextId", "any");
    ...
    void create_shouldNotCreateDefinitionIfTheStoreFails() {
        var definition = createContractDefinition();
        when(store.findById("participantContextId", definition.getId())).thenReturn(null);
    ...
    void delete_shouldDeleteDefinitionIfItsNotReferencedByAnyAgreement() {
        var definition = createContractDefinition();
        when(store.deleteById("participantContextId", "assetId")).thenReturn(StoreResult.success(definition));

        var deleted = service.delete("participantContextId", "assetId");
