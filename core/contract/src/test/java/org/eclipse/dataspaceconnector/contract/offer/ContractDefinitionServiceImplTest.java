...
import static org.eclipse.dataspaceconnector.spi.contract.offer.ContractDefinitionService.OFFER_GENERATION_SCOPE;
...
class ContractDefinitionServiceImplTest {
    ...
    void definitionsFor_verifySatisfiesPolicies() {
        ...
        when(policyEngine.evaluate(OFFER_GENERATION_SCOPE, def.getPolicy(), agent)).thenReturn(Result.success(def.getPolicy()));
        ...
        verify(policyEngine, atLeastOnce()).evaluate(OFFER_GENERATION_SCOPE, def.getPolicy(), agent);
    ...
    void definitionsFor_verifyDoesNotSatisfyAccessPolicy() {
        ...
        verify(policyEngine, atLeastOnce()).evaluate(OFFER_GENERATION_SCOPE, definition.getPolicy(), agent);
        verify(definitionStore).findAll(any());
    }
    ...
    void definitionsFor_verifyPoliciesNotFound() {
        ...
        when(policyEngine.evaluate(OFFER_GENERATION_SCOPE, policy, agent)).thenReturn(Result.success(policy));
    ...
    void definitionFor_found() {
        ...
        when(policyStore.findById(any())).thenReturn(definition);
        when(policyEngine.evaluate(eq(OFFER_GENERATION_SCOPE), isA(Policy.class), isA(ParticipantAgent.class))).thenReturn(Result.success(definition.getPolicy()));
