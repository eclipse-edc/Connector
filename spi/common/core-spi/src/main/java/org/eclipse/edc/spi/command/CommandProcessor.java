...
public class CommandProcessor<C extends Command> {
    ...
    public boolean processCommandQueue(C command) {
        ...
        if (commandResult.failed()) {
            if (command.canRetry()) {
                monitor.warning(String.format("Could not process command [%s], will retry. Error: %s", command.getClass(), commandResult.getFailureMessages()));
                commandQueue.enqueue(command);
            } else {
                monitor.severe(String.format("Could not process command [%s], it has exceeded its retry limit, will discard now. Error: %s", command.getClass(), commandResult.getFailureMessages()));
            }
            return false;
        } else {
