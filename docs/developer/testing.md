...
All integration tests should go into the [verify workflow](IdeaProjects/EclipseDataspaceConnector/.github/workflows/verify.yaml), every
...
The runner can load an EDC runtime by using the `@RegisterExtension` annotation (example in [`FileTransferIntegrationTest`](IdeaProjects/EclipseDataspaceConnector/system-tests/tests/src/test/java/org/eclipse/dataspaceconnector/system/tests/local/FileTransferIntegrationTest.java)).

To make sure that the runtime extensions are correctly built and available, they need to be set as dependency of the runner module as `testCompileOnly`. (example in [`build.gradle.kts`](IdeaProjects/EclipseDataspaceConnector/system-tests/tests/build.gradle.kts)).
