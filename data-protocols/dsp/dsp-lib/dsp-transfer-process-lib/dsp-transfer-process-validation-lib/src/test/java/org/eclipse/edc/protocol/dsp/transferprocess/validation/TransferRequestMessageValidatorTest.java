...
class TransferRequestMessageValidatorTest {

    private static final JsonLdNamespace DSP_NAMESPACE = new JsonLdNamespace("http://www.w3.org/ns/dsp#");
    private final Validator<JsonObject> validator = TransferRequestMessageValidator.instance(DSP_NAMESPACE);

    @Test
    void shouldSucceed_whenObjectIsValid() {
        var input = Json.createObjectBuilder()
                .add(TYPE, Json.createArrayBuilder().add(DSP_NAMESPACE.toIri(DSPACE_TYPE_TRANSFER_REQUEST_MESSAGE_TERM)))
                .build();
