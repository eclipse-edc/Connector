...
class JsonObjectFromTransferProcessV2024TransformerTest {
    ...
    void transform_status(TransferProcessStates inputState, String expectedDspState, TransferProcessStates prevState) {
        var dataAddress = DataAddress.Builder.newInstance()
                .keyName("dataAddressId")
                .property("type", "TestValueProperty")
                .build();
        var transferProcess = TransferProcess.Builder.newInstance()
                ...
                .contentDataAddress(dataAddress)
                .previousState(prevState != null ? prevState.code() : null)
                .build();
    ...
    public static class Status implements ArgumentsProvider {
        ...
        public Stream<? extends Arguments> provideArguments(ExtensionContext context) {
            return Stream.of(
                    ...
                    arguments(DEPROVISIONING, toIri(DSPACE_VALUE_TRANSFER_STATE_COMPLETED_TERM), COMPLETED),
                    arguments(DEPROVISIONING_REQUESTED, toIri(DSPACE_VALUE_TRANSFER_STATE_COMPLETED_TERM), COMPLETED),
                    arguments(DEPROVISIONED, toIri(DSPACE_VALUE_TRANSFER_STATE_COMPLETED_TERM), COMPLETED),
                    arguments(DEPROVISIONING, toIri(DSPACE_VALUE_TRANSFER_STATE_TERMINATED_TERM), TERMINATED),
                    arguments(DEPROVISIONING_REQUESTED, toIri(DSPACE_VALUE_TRANSFER_STATE_TERMINATED_TERM), TERMINATED),
                    arguments(DEPROVISIONED, toIri(DSPACE_VALUE_TRANSFER_STATE_TERMINATED_TERM), TERMINATED),
