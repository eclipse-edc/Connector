    ...
    private <T> StatusResult<T> handleResponse(Response response, String protocol, Class<T> responseType, DspHttpResponseBodyExtractor<T> bodyExtractor) {
        try (var responseBody = response.body()) {
            if (response.isSuccessful()) {
                ...
                return StatusResult.failure(status, "Counter Party responded with %d. Message: %s. Body: %s. Request: %s"
                        .formatted(response.code(), response.message(), asString(responseBody), response.request()));
            }
        }
    }
