...
/**
 * Provides HTTP dispatching for Dataspace Protocol transfer process messages via the {@link ProtocolRemoteMessageDispatcher}.
 */
...
public class DspTransferProcessDispatcherExtension implements ServiceExtension {
    ...
    public void initialize(ServiceExtensionContext context) {
        messageDispatcher.registerMessage(
                ...
                new PostDspHttpRequestFactory<>(remoteMessageSerializer, requestBasePathProvider, m -> BASE_PATH + TRANSFER_INITIAL_REQUEST),
        ...
        messageDispatcher.registerMessage(
                ...
                new PostDspHttpRequestFactory<>(remoteMessageSerializer, requestBasePathProvider, m -> BASE_PATH + m.getProcessId() + TRANSFER_COMPLETION),
                NOOP
        );
        messageDispatcher.registerMessage(
                TransferStartMessage.class,
                new PostDspHttpRequestFactory<>(remoteMessageSerializer, requestBasePathProvider, m -> BASE_PATH + m.getProcessId() + TRANSFER_START),
                NOOP
        );
        messageDispatcher.registerMessage(
                TransferSuspensionMessage.class,
                new PostDspHttpRequestFactory<>(remoteMessageSerializer, requestBasePathProvider, m -> BASE_PATH + m.getProcessId() + TRANSFER_SUSPENSION),
                NOOP
        );
        messageDispatcher.registerMessage(
                TransferTerminationMessage.class,
                new PostDspHttpRequestFactory<>(remoteMessageSerializer, requestBasePathProvider, m -> BASE_PATH + m.getProcessId() + TRANSFER_TERMINATION),
                NOOP
        );
    }
}
