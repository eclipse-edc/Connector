...
import org.eclipse.dataspaceconnector.ids.daps.DynamicAttributeToken;
...
public class DapsClient {
    ...
    // TODO add scopes, for example if a company uses custom scopes within their identity provider
    @NotNull
    public DynamicAttributeToken requestDynamicAttributeToken() throws DapsClientException {
        final String issuer = issuerProvider.getIssuer();
        ...
        final AccessTokenResponse accessTokenResponse = mapResponse(response);

        return createDat(accessTokenResponse);
    }
    ...
    private AccessTokenResponse mapResponse(final Response response) throws DapsClientException {
        ...
        try (final InputStream inputStream = responseBody.byteStream()) {
    ...
    private DynamicAttributeToken createDat(final AccessTokenResponse accessTokenResponse) {
        ...
        return new DynamicAttributeToken(token, expirationDate);
    }
}
